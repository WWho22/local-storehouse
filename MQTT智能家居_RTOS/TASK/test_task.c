...
const char AT_Server_Connect[]     = "AT+CIPSTART=\"TCP\",\"";//ESP8266 设备作为 TCP client 连接到上述服务器                                                      //还要补充服务器地址和端口
...
void AT_TestTask(void* arg)
{
	...
	while(1)
	{
		ret = UART_AT_Send("AT",strlen(AT_Test),200);
		printf("ret is %d\r\n",ret);
	}
}
